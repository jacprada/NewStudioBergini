...
function wat_remove_media_controls() {
     remove_action( 'media_buttons', 'media_buttons' );
}
 add_action('admin_head','wat_remove_media_controls');

/*-----------------------------------------------------------------------------------*/
/* Remove Dashboard widgets */
/*-----------------------------------------------------------------------------------*/
function wat_remove_dashboard_widget() {
...
add_action('wp_dashboard_setup', 'wat_remove_dashboard_widget' );

/*-----------------------------------------------------------------------------------*/
/* Replace Logo on login page */
/*-----------------------------------------------------------------------------------*/
function wat_login_logo() { ?>
    <style type="text/css">
...
<?php }
add_action( 'login_enqueue_scripts', 'wat_login_logo' );

add_filter( 'login_headerurl', 'wat_loginlogo_url' );
function wat_loginlogo_url($url) {
  return get_site_url();
}
...
function wat_remove_my_post_metaboxes() {
  //remove_meta_box( 'categorydiv','post','normal' ); // Categories Metabox
  remove_meta_box( 'tagsdiv-post_tag','post','normal' ); // Tags Metabox
   remove_submenu_page('edit.php', 'edit-tags.php?taxonomy=post_tag');
}
add_action('admin_menu','wat_remove_my_post_metaboxes');
...
function wat_remove_admin_menu_items() {
...
add_action('admin_menu', 'wat_remove_admin_menu_items');

add_action( 'admin_bar_menu', 'wat_remove_wp_nodes', 999 );
//remove from the NEW bar
function wat_remove_wp_nodes() 
{
    global $wp_admin_bar;   
...
function wat_remove_footer_admin () {
    echo "<?php
...
add_filter('admin_footer_text', 'wat_remove_footer_admin');


//remve meta boxes 
function wat_remove_meta_boxes() 
{
...
add_action( 'add_meta_boxes', 'wat_remove_meta_boxes', 999 );

/* Remove unwanted columns from post */
function wat_manage_columns( $columns ) {
  unset($columns['comments']);
  return $columns;
}

function wat_column_init() {
  add_filter( 'manage_posts_columns' , 'wat_manage_columns' );
  add_filter( 'manage_pages_columns' , 'wat_manage_columns' );
}
add_action( 'admin_init' , 'wat_column_init' );
